...
3. Client not connected → `manage_test_client_profiles` (`action=get_list`) to read the profile name, then `manage_test_client` (`action=connect`) with that exact `profileName`. Do not hardcode a profile.
...
- Typical library-step examples (confirm each via search; command/window titles come from the configuration under test):
