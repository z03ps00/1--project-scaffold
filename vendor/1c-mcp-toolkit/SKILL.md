...
1. Карта портов — `USER-RULES.md` (секция «Конвертация данных» / «MCP Toolkit»)
   и `.dev.env`: `MCP_TOOLKIT_PORT`, `KD2_PORT`, `KD31_PORT`. Есть карта —
   health-probe по этим портам, не спрашивать.
2. Health-probe вместо вопросов:

```sh
bash scripts/health-probe.sh
# или: MCP_TOOLKIT_PORT=7003 bash scripts/health-probe.sh
```
...
3. Ничего не живо — **не** запускать 1С скриптом за пользователя. Сказать открыть
   ...
   [`docs/integration.md`](docs/integration.md). `_docs/` is for user documents, not this guide.
...
`vcexecutequery` / `vcexecutecode`. По умолчанию toolkit не подменять.
...
- `scripts/health-probe.sh` — обход портов из `.dev.env` / типичных 6003, 7003, 6011, …
- `scripts/query.sh "<запрос>" [out.json]` — `execute_query` (`MCP_TOOLKIT_PORT`, дефолт 6003)
- `scripts/exec.sh <файл.bsl> [out.json]` — `execute_code` (результат в `Результат`, не `Возврат`)

Кириллица: ответ в файл, читать через Read.
